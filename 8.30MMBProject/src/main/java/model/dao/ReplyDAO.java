...
//	//페이지 범위, 댓글 고유번호, 내용, 닉네임, 프로필경로, 작성일자, 댓글 페이지(시작~끝) 조회
	...
	// SELECTALL 특정 게시물 댓글 모두 조회
	private final String SELECTALL = "SELECT RN, REPLY_NUM, REPLY_CONTENT, MEMBER_NICKNAME, MEMBER_PROFILE_WAY, REPLY_WRITE_DAY"
	...
	private final String MEMBER_REPLY_SELECTONE = "SELECT R.REPLY_NUM, B.BOARD_TITLE, R.REPLY_CONTENT, M.MEMBER_NICKNAME, R.REPLY_WRITE_DAY"
	+"FROM BD_REPLY R JOIN BD_MEMBER M ON M.MEMBER_NUM = R.MEMBER_NUM"
	+"JOIN BD_BOARD B ON B.BOARD_NUM =R.BOARD_NUM WHERE R.MEMBER_NUM = ? AND R.BOARD_NUM =?";
	...
	public boolean insert(ReplyDTO replyDTO) {// 댓글 추가 기능
		...
		try {
			...
			pstmt.setString(1, replyDTO.getReplyContent()); // 이메일 입력
			pstmt.setInt(2, replyDTO.getMemberNum()); // 비밀번호 입력
			pstmt.setInt(3, replyDTO.getBoardNum()); // 이름 입력
			...
			System.err.println("log: ReplyDTO insert SQLException fail");
			return false;
		} finally {
			// 연결해제
	...
	public ArrayList<ReplyDTO> selectAll(ReplyDTO replyDTO) {// 게시물 댓글 전체 조횐
		...
		try {
			...
			data.setPageStartNum(rs.getInt("REPLY_PAGE_START")); // 댓글 시작페이지 넣기
			data.setPageEndNum(rs.getInt("REPLY_PAGE_END"));// 댓글 마지막 페이지 넣기
	...
	public boolean update(ReplyDTO replyDTO) {
		...
		try {
			...
			System.err.println("log: ReplyDTO update SQLException fail");
			return false;
		} finally {
			// 연결해제
			if (!JDBCUtil.disconnect(conn, pstmt)) {// 만약 jdbc가 연결되어 있다면
	...
	public boolean delete(ReplyDTO replyDTO) {
		...
		try {
			...
			System.err.println("log: ReplyDTO delete SQLException fail");
			return false;
		} finally {
			// 연결해제
			if (!JDBCUtil.disconnect(conn, pstmt)) {// 만약 jdbc가 연결되어 있다면
