...
public class MemberDAO {
   ...
   final String MEMBER_DETAIL_SELECTONE = "SELECT MEMBER_NUM FROM BD_MEMBER WHERE MEMBER_NUM =?";

   // 패스워드 일치 여부 확인 회원 1명 조회. 인풋값 PK,패스워드 통해서 고유번호, 이메일, 이름, 폰번호, 닉네임, 프로필사진 경로, 역할
   // 모두 반환
   final String PASSWORD_CHEK_SELECTONE = "SELECT MEMBER_NUM FROM BD_MEMBER WHERE MEMBER_NUM = ? AND MEMBER_PW = ?";

   // 특정 회원 프로필 사진 경로 조회 : PK값 C에서 주면, M은 특정 회원 조회 + 사진 경로 반환
   final String PROFILEWAY_SELECTONE = "SELECT MEMBER_PROFILE_WAY FROM BD_MEMBER WHERE MEMBER_NUM = ?";
...
//MEMBER_DETAIL_SELECTONE ----------------------------------------------------------------            
            ...
            if(rs.next()){
               ...
               data.setMemberEmail(rs.getString("MEMBER_EMAIL"));         //회원 이메일 입력
               data.setMemberNum(rs.getInt("MEMBER_NICKNAME"));         //회원 닉네임 입력
               data.setMemberNum(rs.getInt("MEMBER_PHONE"));            //회원 전화번호 입력
               data.setMemberNum(rs.getInt("MEMBER_PROFILE_WAY"));         //회원 프로필사진경로 입력
               data.setMemberNum(rs.getInt("MEMBER_ROLE"));            //회원 권한 입력
               data.setMemberNum(rs.getInt("MEMBER_HIREDAY"));            //회원 가입일자 입력
            }
//PASSWORD_CHEK_SELECTONE ----------------------------------------------------------------               
         }else if(memberDTO.getCondition().equals("PASSWORD_CHEK_SELECTONE")){
            System.out.println("log: MemberDAO selectOne condition : PASSWORD_CHEK_SELECTONE");
            pstmt = conn.prepareStatement(PASSWORD_CHEK_SELECTONE);         //SQL DB에서 PASSWORD_CHEK_SELECTONE 준비
            pstmt.setInt(1, memberDTO.getMemberNum());                   //고유번호 입력
